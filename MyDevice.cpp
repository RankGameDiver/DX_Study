...
namespace MyDevice
{
	...
	D3DPRESENT_PARAMETERS GetD3DPP(bool iswindow)
	{
		...
		d3dpp.BackBufferWidth = WINSIZE_WIDTH;							//백버퍼 픽셀 가로 크기
		d3dpp.BackBufferHeight = WINSIZE_HEIGHT;						//백버퍼 픽셀 높이 크기
		...
		d3dpp.hDeviceWindow = _hWnd;									//Device 가 출력될 윈도우 핸들
		...
		d3dpp.Windowed = iswindow;										//창모드 / 전체화면
